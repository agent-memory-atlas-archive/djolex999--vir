...
import { DISTILL_DIR, DISTILL_GRADING_DIR } from "./paths.js";
...
// grades are complete.
export async function runJudge(model: string = "claude-sonnet-5"): Promise<void> {
  const DISTILL_JUDGE_PATH = join(DISTILL_DIR, judgeFileName(model));
  ...
  process.stdout.write(`judge preview (${model}): ${todo.length} notes to score (${have.size} cached)\n`);
  const fresh = await mapLimit(todo, 3, async (id) => {
    const body = readFileSync(join(DISTILL_GRADING_DIR, `${id}.md`), "utf8");
    const res = await callJudge("eval-distill-judge", buildNoteJudgePrompt(rubric, body), id, model);
  ...
  writeFileSync(DISTILL_JUDGE_PATH, JSON.stringify({ version: 1, rubricSha256, model, grades: [...existing, ...fresh] }, null, 2));
